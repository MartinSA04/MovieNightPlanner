# Movie Night Planner — Claude Guide
...
* `apps/web/scripts/seed-test-data.mjs`: seeds local Supabase with test users/groups/events
...
* Seed test data: `corepack pnpm db:seed:test` (creates alice/bob/carol/dave/erin @test.movienight.local, password `password123`)
...
* Keep sensitive writes in trusted server-side code — always route privileged DB work through the service-role admin client (`apps/web/lib/supabase/admin.ts`). Anon/auth clients are for user-scoped reads under RLS.
...
* Ranked votes: up to 3 ordered picks per user per event, worth 3/2/1 points.
...
* `movie_suggestions` inserts and `votes` writes are allowed while the event is `draft` or `open`.

## Database & RLS

* Schema lives in `supabase/migrations/`. New policies should wrap `auth.uid()` in `(select auth.uid())` to avoid per-row re-evaluation.
* Do not use `FOR ALL` on write policies when a dedicated `FOR SELECT` policy exists — split into explicit `INSERT/UPDATE/DELETE` policies so you don't end up with multiple permissive SELECT policies.
* Cache tables (`movie_cache`, `watch_provider_cache`, `streaming_services`) are service-role only; RLS is enabled with no policies so regular clients can't read them.
* `set_updated_at` and any other function should pin `set search_path = public` to satisfy the Supabase linter.

## UI/UX and Design Principles

All UI work must prioritize clean, modern, and minimal design. The product should feel simple, polished, and easy to use at every step.
...
* Themes are class-driven via the `dark` class on `<html>` and CSS variables in `app/globals.css`.
* Keep the `:root` (light) and `html.dark` palettes distinct — if they duplicate, the toggle becomes a no-op.
* All color utilities should reference tokens (`bg-card`, `text-foreground`, `border-border/60`, …), never hard-coded hex.
...
* When a panel must scroll internally *and* not grow the page's scroll height, `overflow-y-auto + max-height` is not enough on its own. Use a `relative h-[X] overflow-hidden` shell with an `absolute inset-0 overflow-y-auto` inner scrollport so the inner content is out of flow.
